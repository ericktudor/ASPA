...
// 13/nume.prenume/tel/loc/descriere/

string ProtocolReportEvent::handleMessage(string msg)
{
	int x = msg.find("/");
	int y = msg.find("/", x + 1);
	...
	this->description = msg.substr(x + 1, y - x - 1);
	cout << this->description << endl;
	cout << x << " " << y << endl;


	//INSERT INTO Events(Name, Surname, Telephone, Location, Description) VALUES(@this->name, @this->surname, @this->tel, @this->location, @this->description)
    ...
    do {
        ...
        strcpy(newvar1, this->name.c_str());
        char* newvar2 = new char[50];
        strcpy(newvar2, this->surname.c_str());
        char* newvar3 = new char[50];
        strcpy(newvar3, this->tel.c_str());
        char* newvar4 = new char[50];
        strcpy(newvar4, this->location.c_str());
        char* newvar5 = new char[50];
        strcpy(newvar5, this->description.c_str());
        SQLCHAR newquery[1000];

        sprintf((char*)newquery, "INSERT INTO Events(Name, Surname, Telephone, Location, Description) VALUES('%s','%s','%s','%s','%s')", newvar1, newvar2, newvar3, newvar4, newvar5);
        ...
        if (SQL_SUCCESS != SQLExecDirect(SQLStatementHandle, (SQLWCHAR*)instr.c_str(), SQL_NTS))
        {
            ...
            cout << "\n";
            cout << "Evenimentul nu a fost inregistrat cu succes cu succes!\n";
            this->msg = "*0*";
        }
        else
        {
            cout << "Evenimentul a fost inregistrat cu succes cu succes!\n";
            this->msg = "*1*";
        }
	...
	LOG::getInstance().logReportEvent();
