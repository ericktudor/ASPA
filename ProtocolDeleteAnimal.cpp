...
string ProtocolDeleteAnimal::handleMessage(string msg)
{
	int x = msg.find("/");
	int y = msg.find("/", x + 1);
	...
	//delete in baza de date a animalului cu ID_ul dat

	//DELETE from Animals WHERE ID_animal=this->ID_animal
    ...
    do {
        ...
        sprintf((char*)newquery, "DELETE from Animals WHERE ID_animal=%s", newvar1);
        ...
        {
            this->msg = "*1*";
            cout << "Stergerea animalului din baza de date realizata cu succes!\n";
        }
    } while (FALSE);
	...
	LOG::getInstance().logDeleteAnimal();
