...
string ProtocolShowAnimals::handleMessage(string msg)
{
	int x = msg.find("/");
	int y = msg.find("/", x + 1);
	...
	//de extras din baza de date si de split uit datele citite de acolo
    ...
    do {
        ...
        {
            // 34/age/sex/location/size/species/
	...
	{
		cout << "Au fost transmise datele cerute despre animalele existente!\n";
		this->msg = "1/1/3/Mascul/Pallady/Medie/Caine/";
	}
	else
	{
		cout << "Nu s-au gasit date cu particularitatile cerute!\n";
		this->msg = "*0*";
	}*/

	LOG::getInstance().logShowAnimals();
